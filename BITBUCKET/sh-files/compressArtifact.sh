        ...
        #####################################################################################################
         ...
         functioncompresstar(){
            ...
            if [ -z "$WorkSpaceTrigger" ]; then # Si no se envia carpeta de repositorio de la aplicacion
                ...
                #Creacion de carpeta Artifact changeNumber, leer archivo de archviso y copiar en la nueva carpeta    
                    mkdir -p "$pathArtifact"
                    #echo "mkdir -p $pathArtifact"
                ...
                #Copiar Archivos de Bistbucket a Carpeta del Pipeline
                    cp --parents $(cat "$tmpReport") "$pathArtifact"
                    #echo "cp -r $(cat $tmpReport) $pathArtifact"

                #Cambio directorio reducir rutas de comprimido
                    cd "$pathArtifact/"
                    #pwd

                    validate="$( find . -name "*$patternFiles" | wc -l )"
