			...
			if [ $(echo $?) -eq 2 ]; # Si no puede abrir el .log entonces arroja error
			 then
				...
				if [ -f "$log_report" ]; then
					if [ $(grep -c -E ":: failed imported|:: already exists|:: Resource failed imported."  $log_report) -gt 0 ]; then
						grep -w -i -E ":: failed imported|:: already exists|was not found."  $log_report >> $log_error
